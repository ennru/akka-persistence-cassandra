...
import com.datastax.oss.driver.api.core.cql.{ BoundStatement, PreparedStatement, Row, Statement }
...
private[akka] trait CassandraEventUpdate extends CassandraStatements {
  ...
  def psUpdateMessage: Future[PreparedStatement] =
    session.prepare(updateMessagePayloadAndTags)
  //.map(_.setIdempotent(true))
  def psSelectTagPidSequenceNr: Future[PreparedStatement] =
    session.prepare(selectTagPidSequenceNr)
  //.map(_.setIdempotent(true))
  def psUpdateTagView: Future[PreparedStatement] =
    session.prepare(updateMessagePayloadInTagView)
  //.map(_.setIdempotent(true))
  ...
  private def prepareUpdate(ps: PreparedStatement, s: Serialized, partitionNr: Long): BoundStatement = {
    val bs = ps.bind()
