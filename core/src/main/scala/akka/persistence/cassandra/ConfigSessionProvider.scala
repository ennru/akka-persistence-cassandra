...
class ConfigSessionProvider(system: ActorSystem, config: Config) extends SessionProvider {
  ...
  def connect()(implicit ec: ExecutionContext): Future[CqlSession] = {
    ...
    clusterBuilder(clusterId).map { b =>
      val cluster = b.build()
...
//      case "" => None
//      case _  => Some(DefaultProtocolVersion.valueOf(config.getString("protocol-version")))
//    }
...
//
  def clusterBuilder(clusterId: String)(implicit ec: ExecutionContext): Future[CqlSessionBuilder] = {
    lookupContactPoints(clusterId).map { cp =>
      val b = CqlSession.builder
      b
    }
  }
//          .withConfigLoader(new DefaultDriverConfigLoader(supplier))
